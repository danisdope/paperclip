...
const CLOSEOUT_SECTION_RE = {
  changed: /(?:^|\n)\s*(?:#{1,6}\s*)?What changed\s*:?\s*(?:\n|$)/i,
  passed: /(?:^|\n)\s*(?:#{1,6}\s*)?What passed\s*:?\s*(?:\n|$)/i,
  followUp: /(?:^|\n)\s*(?:#{1,6}\s*)?What still needs follow-up\s*:?\s*(?:\n|$)/i,
};
...
function hasRequiredCloseoutSections(body: string): boolean {
  return (
    CLOSEOUT_SECTION_RE.changed.test(body) &&
    CLOSEOUT_SECTION_RE.passed.test(body) &&
    CLOSEOUT_SECTION_RE.followUp.test(body)
  );
}
...
export async function assertIssueCanMoveToDone(db: DbOrTx, issue: IssueWithPolicy) {
  ...
  if (!hasRequiredCloseoutSections(closeout.body)) {
    throw unprocessable("Done blocked: closeout must include What changed, What passed, and What still needs follow-up", {
