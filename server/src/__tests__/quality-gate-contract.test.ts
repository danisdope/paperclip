...
function approvedDecision(stageId: string) {
  return {
    id: `${stageId}-decision`,
    stageId,
    outcome: "approved",
...
function makeDb(input: {
  ...
  decisions?: Array<{ id: string; stageId: string; outcome: string }>;
}) {
  return {
    select: () => {
      const builder = {
    ...
    await expect(
      assertApprovalMergeGateReadyForIssueIds(
        makeDb({
          ...
          issueRows: [makeIssue()],
          decisions: [approvedDecision(adversarialStageId), approvedDecision(codeReviewStageId)],
        }),
        { payload: { stage: "merge_gate" } },
        [issueId],
      ),
